/*=========================================================================
  ...
  Module    : $Id: irtkScalarGaussianDzDz.cc 62 2009-05-28 13:19:03Z dr $
...
irtkScalarGaussianDzDz::irtkScalarGaussianDzDz(double _sigma_x, double _sigma_y, double _sigma_z, double x_0, double y_0, double z_0)
{
  _Sigma_x = _sigma_x;
  _Sigma_y = _sigma_y;
  _Sigma_z = _sigma_z;
  ...
  if ((_Sigma_x != 0) && (_Sigma_y != 0) && (_Sigma_z != 0)){
    _Factor = 1.0 / (sqrt(2.0 * M_PI) * _Sigma_x * 
	  ...
	  cerr << "irtkScalarGaussianDzDz::irtkScalarGaussianDzDz: Warning, divide by zero" << endl;
    _Factor = 0.0;
  }
}
...
double irtkScalarGaussianDzDz::Evaluate(double x, double y, double z)
{
  ...
  return - (1.0 / _VarZ) * _Factor * _Exp + (z - _Z_0) * (1.0 / _VarZ) * (z - _Z_0) * (1.0 / _VarZ) * _Factor * _Exp;
}
