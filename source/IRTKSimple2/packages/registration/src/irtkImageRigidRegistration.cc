...
void irtkImageRigidRegistration::GuessParameterSliceToVolume(bool useNMI)
{
  int i;
  double xsize, ysize, zsize;
...
double irtkImageRigidRegistration::Evaluate()
{
  ...
  //TODO CUDA
  //deactivate TBB
  //copy target and source (maybe fix data cpy via cc)
  //perform CC in grid size target (or smaller volume)
  //evaluate every voxel of target and its source.WorldToIMage()*target.ImageToWorld()*targetPos.vec3
